/**
 * جستجوی تصویر آموزشی از ویکی‌مدیا کامنز (و ویکی‌پدیا فارسی) — فقط سمت سرور.
 * هیچ‌وقت خطا پرتاب نمی‌کند؛ اگر چیزی پیدا نشود null برمی‌گرداند.
 */
...
const EN_BLOCK =
  /\b(nud(e|ity)|sex(ual|y)?|erotic|porn\w*|genital\w*|penis|vagina|breasts?|bikini|lingerie|gore|corpse|torture|rape|nazi|swastika|isis|suicide|lynching|abuse)\b/i;
const FA_BLOCK = /(برهنه|پورن|سکس|آلت تناسلی|شهوت|جسد|اعدام|تجاوز|خودکشی)/;
...
async function getJson<T>(url: string, ms: number): Promise<T | null> {
  ...
  try {
    ...
    if (!r.ok) {
      console.error("[pouya-ai] wiki http", r.status, url.slice(0, 90));
      return null;
    }
    return (await r.json()) as T;
  } catch (err) {
    console.error("[pouya-ai] wiki exception", err instanceof Error ? err.message : String(err));
    return null;
  } finally {
    clearTimeout(t);
  }
...
async function commonsImage(q: string): Promise<WikiImage | null> {
  ...
  for (const p of pages) {
    ...
    return {
      url: safeUrl(thumb),
      alt: cleanLabel(name, 60) || "تصویر آموزشی",
      label: `ویکی‌مدیا کامنز — ${cleanLabel(name, 60)}${artist ? " — " + artist : ""} — ${cleanLabel(license, 30)}`,
    };
  }
  return null;
}
...
async function faWikiImage(q: string): Promise<WikiImage | null> {
  ...
  for (const p of pages) {
    ...
    // فقط فایل‌های آزاد کامنز؛ نه تصاویر «استفاده منصفانه» محلی ویکی‌پدیا
    ...
    return {
      ...
      label: `ویکی‌پدیای فارسی — ${cleanLabel(p.title, 60)}`,
    };
  }
  return null;
}
...
/** تصویرهای مارک‌داون غیر ویکی‌مدیا را از پاسخ مدل حذف می‌کند (جلوگیری از ردیابی/نشت اطلاعات). */
export function stripForeignImages(text: string): string {
  return text.replace(/!\[[^\]]*\]\(([^)]*)\)/g, (m, url: string) => (url.trim().startsWith(UPLOAD) ? m : ""));
}

/** تگ‌های [wiki:عبارت] مدل را با تصویر واقعی جایگزین می‌کند. اولین تگ تصویر می‌گیرد، بقیه حذف می‌شوند. */
...
const VISUAL_RE = /(عکس|تصویر|شکل|نمودار|برش|نقشه|ساختار|اجزا|نشان\s*بده|نشونم|ببینم|دیاگرام|طرح)/;
export function looksVisual(text: string): boolean {
  return VISUAL_RE.test(text);
}
...
const FA_EN: [string, string][] = [
  ...
  ["دستگاه ادراری", "urinary system"], ["جدول تناوبی", "periodic table"], ["چرخه آب", "water cycle"],
  ["سلول گیاهی", "plant cell"], ["سلول جانوری", "animal cell"], ["دی ان ای", "DNA"], ["فتوسنتز", "photosynthesis"],
  ["برش افقی", "axial section"], ["برش عرضی", "cross section"], ["برش طولی", "longitudinal section"],
  ["برش کرونال", "coronal section"], ["برش ساژیتال", "sagittal section"], ["برش", "section"],
  ["مغز", "brain"], ["قلب", "heart"], ["ریه", "lung"], ["کلیه", "kidney"], ["کبد", "liver"], ["معده", "stomach"],
  ["روده", "intestine"], ["استخوان", "bone"], ["ماهیچه", "muscle"], ["عضله", "muscle"], ["سلول", "cell"],
  ["نورون", "neuron"], ["چشم", "eye anatomy"], ["گوش", "ear anatomy"], ["دندان", "tooth anatomy"], ["پوست", "skin layers"],
  ["جمجمه", "skull"], ["اسکلت", "skeleton"], ["اتم", "atom"], ["مولکول", "molecule"], ["برگ", "leaf anatomy"],
  ["ریشه", "root anatomy"], ["گل", "flower anatomy"], ["خورشید", "sun"], ["ماه", "moon"], ["زمین", "earth"],
  ["آتشفشان", "volcano"], ["زلزله", "earthquake"], ["موج", "wave"], ["مدار", "electric circuit"], ["مثلث", "triangle"],
  ["نقشه", "map"],
];
...
/**
 * از متن فارسی کاربر یک عبارت جستجو می‌سازد. اگر اصطلاح علمی شناخته‌شده باشد انگلیسی می‌شود.
 * loose=false: فقط وقتی متن «بصری» به نظر برسد یا اصطلاح شناخته‌شده داشته باشد.
 */
export function queryFromPersian(text: string, loose = false): string | null {
  ...
  for (const [fa, en] of keys) {
    ...
    if (k.includes(" ")) {
      ...
      const toks = t.split(" ");
      const idx = toks.findIndex((x) => x === k || x === k + "ها" || x === k + "های" || x === k + "ی" || x === k + "ام");
      if (idx >= 0) {
        hit = true;
        toks.splice(idx, 1);
        t = toks.join(" ");
