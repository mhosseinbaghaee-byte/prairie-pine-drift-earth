      ...
      <div ref={scrollerRef} className="min-h-0 flex-1 overflow-y-auto px-3 py-4 sm:px-5">
        {empty ? (
          <div className="mx-auto flex max-w-xl flex-col gap-6 pt-2">
            ...
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" size="sm" onClick={onLivePractice}>
      ...
      >
        <div className="mx-auto flex max-w-xl items-end gap-2">
          <Button
            ...
            onClick={onMic}
            disabled={busy}
            aria-label="گفتگوی زنده با میکروفون"
            aria-pressed={listening}
            title={listening ? "توقف گوش دادن" : "صحبت کن"}
            className={listening ? "bg-stage-deep text-cream hover:bg-stage" : undefined}
          >
        ...
        <div className="mx-auto flex max-w-xl items-end gap-2">
          ...
          <Textarea
            ...
            placeholder={
              listening
                ? "دارم گوش می‌دهم…"
                : `به ${currentLang?.native ?? "English"} یا فارسی بنویس…`
            }
            className="max-h-32 min-h-12 flex-1 rounded-lg py-3"
            disabled={busy}
          />
